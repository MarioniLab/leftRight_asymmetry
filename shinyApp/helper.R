...
plotlyGeneOnUMAP_lr <- function(gene="Nodal") {
  id <- row.names(ann[ann$gene==gene,])
  df <- data.frame(x = clusters$x, y = clusters$y, cluster = clusters$population, 
                   side = meta[match(row.names(clusters), meta$cellID),'side'],
                   log2.exp = countsNorm[id,row.names(clusters)])
  df <- df[order(df$log2.exp),]
...
expr_boxplot_lr <- function(gene="Nodal") {
  ...
  p <- ggplot(df, aes(cluster, log2.exp, colour=cluster, fill=cluster)) +
