...
shinyServer(
  ...
  function(input, output, session) {
    ...
    updateSelectizeInput(session = session, inputId = 'gene_ref', choices = selectList_ref, 
                         selected = 'Nkx2-5', server = TRUE)
    updateSelectizeInput(session = session, inputId = 'gene_lr', choices = selectList, 
                         selected = 'Nodal', server = TRUE)
    ...
    ## geneExpression per side
    output$expression_UMAPplot_lr <- renderPlotly({
      validate(
        need( input$gene_lr %in% ann$gene, '')
      )
      plotlyGeneOnUMAP_lr(gene=input$gene_lr)
    })
    
    output$expression_boxplot_lr <- renderPlotly({
      validate(
        need( input$gene %in% ann$gene, '')
      )
      expr_boxplot_lr(gene=input$gene_lr)
    })
